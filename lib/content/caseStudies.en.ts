...
const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    solutionWalkthrough: {
      ...
      steps: [
        {
          title: "Steps 1–2 · Build the batch, then choose where it lands",
          body: "The operator names the event and searches to add existing dishes one at a time — but for a change this size, adding 77 rows by hand is the wrong tool, so the same screen offers a CSV template to download, edit offline and re-upload. Re-uploading fills the batch and sorts every row automatically: 40 New, 25 Updated, 12 Disable.\n\nNext, Foodpanda and UberEats are selected. Rather than setting a markup branch by branch, the channel settings panel offers a scope choice — unique settings per branch, or one override applied to every selected branch, here 5% on Foodpanda. Branches sit in collapsible location accordions with a search bar, so \"Select all\" takes all 18 in one action while existing per-branch overrides, like Taichung's NT$15 surcharge, stay locked.",
          image: "/work/menu-sync/solution-part1.png",
          imageW: 2600,
          imageH: 1819,
        },
        {
          title: "Steps 3–5 · Schedule, preview, push, then read the result",
          body: "The rollout is either pushed immediately or staged for a specific date and time — staging is what makes a 36-pair operation reversible, since nothing goes live until the scheduled moment and the whole thing cancels as one unit. Before anything is sent, the operator scrolls the preview: how many items, how many branch–channel pairs, and which branches carry overrides that will be preserved.\n\nAfter the push, the report fills in real numbers: 2,580 of 2,772 item-syncs succeeded, 144 overridden, 12 conflicts, and 3 of 36 pairs failed outright. Each row is one branch–channel pair, so a failure is always attributable to a specific branch on a specific platform, and \"View Detail\" opens the full 77-item breakdown with master price against channel price on every row.",
          image: "/work/menu-sync/solution-part2.png",
          imageW: 2600,
          imageH: 1806,
        },
      ],
