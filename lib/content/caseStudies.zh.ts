...
const caseStudies: CaseStudy[] = [
  {
    ...
    problem: {
      paragraphs: [
        "缺乏每期招生報名資料的直接檢視方式：使用者希望能直接在平台內查看招生表現與學生檔案，以簡化工作流程。",
        "介面結構不清楚：頂端導覽不夠清晰，未來也難以容納更複雜的功能；所有欄位都被列在同一頁面上。",
      ],
  ...
  },
  {
    ...
    problem: {
      paragraphs: [
        "許多客戶端的管理員想在課程頁面新增內容，卻擔心會破壞版面，或需要仰賴我們團隊協助處理小幅修改，這對客戶與我們的開發團隊都造成不便。",
      ],
  ...
  },
  {
    ...
    problem: {
      paragraphs: [
        "我們的客戶推出了一套治理能力評估框架，用來評估主管的治理技能，但由於缺乏誘因，參與率一直偏低。",
      ],
    ...
    process: {
      ...
      steps: [
        ...
        {
          title: "2. 快速製作原型以取得回饋",
          body: "我建置了互動原型，用來蒐集回饋並與利害關係人一起驗證使用者流程，這有效協助管理了利害關係人的期望——測試彈出視窗（Modal）如何提供額外說明並避免使用者出錯、確保使用者在每個步驟都感到被引導，並運用一致的元件系統，同時簡化原型與未來的開發交接。",
