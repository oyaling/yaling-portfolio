...
const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    extraSections: [
      {
        heading: "流程 D —— 批次上架：報告設計",
        intro:
          "步驟 5 是迭代最多的地方。摘要必須同時回答三個不同的問題，因此我把先前混在一起的三個層級拆開。",
        items: [
          {
            title: "用同一個摘要元件，同時服務「排程中」與「已上線」",
            body: "同步狀態描述整次上架——排程中或已上線。明細狀態描述品項——已同步、已覆寫、衝突。分店類型描述一組「分店 × 通路」配對——失敗或連線中斷。\n\n一旦這三個層級被命名，同一個摘要元件就能同時服務兩種狀態。左側的「排程中」顯示已排程的數量，其餘尚未發生的以破折號呈現，並說明整次上架仍可重新排程或取消；右側的「已上線」則填入真實結果，並把失敗的配對凸顯為需要處理的重點。",
