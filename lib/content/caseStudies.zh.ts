...
const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    userStory: "",
    summary:
      "這是一份自主進行的案例研究，探討餐飲業者如何讓同一份菜單在多個訂餐平台之間保持一致。業者能輕鬆管理 18 間分店的菜單：情境是同時新增 40 道菜、更新 25 道、下架 12 道，接著確認同步狀況——若有分店失敗，也知道該如何解決。\n\n**在設計流程中運用 Claude Code，我完成研究並串接了 Figma MCP，在四天內於 Figma 產出低擬真與高擬真設計。這是一個從 0 到 1 的挑戰。我也為這段複雜的批次操作流程建立了一個可實際運作的程式原型，讓團隊在內部討論與迭代時有具體的依據。**",
    problem: {
      items: [
        {
    ...
    finalSolution: {
      heading: "深入了解其他流程",
      intro: "其餘流程在原型中的樣貌。點擊任一張圖片可放大檢視。",
      items: [
        {
          title: "流程 D —— 批次上架：報告設計",
          body: "步驟 5 是迭代最多的地方。摘要必須同時回答三個不同的問題，因此我把先前混在一起的三個層級拆開。同步狀態描述整次上架——排程中或已上線。明細狀態描述品項——已同步、已覆寫、衝突。分店類型描述一組「分店 × 通路」配對——失敗或連線中斷。\n\n一旦這三個層級被命名，同一個摘要元件就能同時服務兩種狀態。左側的「排程中」顯示已排程的數量，其餘尚未發生的以破折號呈現，並說明整次上架仍可重新排程或取消；右側的「已上線」則填入真實結果，並把失敗的配對凸顯為需要處理的重點。",
        ...
        {
          title: "流程 E —— 同步總覽儀表板",
          body: "以三個分頁呈現全系統的同步健康狀況：排程中、已上線與失敗。這裡最困難的決定，是把單筆編輯與批次上架放進同一張表格。一次批次上架是一列，卻代表 36 組「分店 × 通路」配對；一筆單筆編輯則是一列代表它自己。若不加區分地混在一起，數字就失去意義了。\n\n有兩件事解決了它。「類型」欄位將每一列標示為批次或單筆，讓讀者永遠知道一列代表什麼。而在「失敗」分頁中，批次列會以手風琴方式展開，列出實際失敗的「分店 × 平台」配對——因為批次推送對每一組配對而言是一筆交易，所以重試必須作用在配對上，而不是品項上。",
