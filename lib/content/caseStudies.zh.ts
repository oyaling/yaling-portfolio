...
const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    solutionWalkthrough: {
      ...
      steps: [
        {
          title: "步驟 1–2 · 建立批次，再決定要推送到哪裡",
          body: "業者為這次異動命名，並透過搜尋逐一加入既有菜色——但以這種規模的異動來說，手動新增 77 列並不是對的工具，因此同一個畫面也提供 CSV 範本，可下載、離線編輯後再重新上傳。重新上傳會填入整個批次，並自動將每一列分類：40 筆新增、25 筆更新、12 筆下架。\n\n接著選取 Foodpanda 與 UberEats。與其逐間分店設定加價，通路設定面板提供了範圍選擇——各分店獨立設定，或是一組覆寫套用到所有已選分店，這裡是 Foodpanda 加價 5%。分店則收納在可展開的地區手風琴選單中並附搜尋列，因此「全選」能一次選取全部 18 間，而既有的分店覆寫設定（例如台中的 NT$15 加價）仍維持鎖定。",
          image: "/work/menu-sync/solution-part1.png",
          imageW: 2600,
          imageH: 1819,
        },
        {
          title: "步驟 3–5 · 排程、預覽、推送，然後解讀結果",
          body: "這次上架可以立即推送，或排程於指定日期與時間——排程正是讓 36 組配對的操作得以復原的關鍵，因為在排定時間到來前不會有任何內容上線，整個操作也能被當作單一單位取消。在任何內容送出之前，業者會逐項檢視預覽：多少品項、多少組「分店 × 通路」配對，以及哪些分店帶有將被保留的覆寫設定。\n\n推送完成後，報告會填入真實數據：2,772 筆品項同步中有 2,580 筆成功、144 筆被覆寫、12 筆衝突，36 組配對中有 3 組完全失敗。每一列代表一組「分店 × 通路」配對，因此失敗永遠能歸因到特定分店的特定平台；點擊「查看明細」則會展開完整的 77 項品項細節，每一列都並列主資料價格與通路價格。",
